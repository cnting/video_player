...
class MethodChannelVideoPlayer extends VideoPlayerPlatform {
  ...
  Stream<VideoEvent> videoEventsFor(int textureId) {
    return _eventChannelFor(textureId).receiveBroadcastStream().map((dynamic event) {
      ...
      switch (map['event']) {
        ...
        case 'downloadState':
          return VideoEvent.forDownload(map['state'], map['progress']);
        default:
  ...
  Future<void> download(int textureId, int trackIndex, String name) {
    print('===>download 11111');
    return _api.download(DownloadMessage()
      ..textureId = textureId
      ..name = name);
  }

  Future<void> removeDownload(int textureId) {
    return _api.removeDownload(TextureMessage()..textureId = textureId);
  }
